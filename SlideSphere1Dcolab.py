...
from Model import Model  # make sure model.py is uploaded to Colab
...
# ======================================================
def main():
    # Colab does not have __file__, so set root manually
    REPO_ROOT = "/content"  # <-- adjust if needed
    ...
    for i in range(N):
        ...
        for j in range(i+1, N):
            ...
            if direction * (zj - zi) < 0:
                violations += 1
            total += 1
        disorder_percent.append(100 * violations / total if total > 0 else 0)
    ...
    ax.grid(True)
    fig.tight_layout()
    fig.savefig(os.path.join(PLOT_DIR, "latent_disorder_curve.png"), dpi=150)
    plt.close(fig)
    ...
    edges = {"left": latent_min.to(device), "right": latent_max.to(device)}
    ...
    print(f"[METRIC] Linear regression R^2: {r2:.4f}")
...
# ======================================================
# This block evaluates the sensitivity of the model's output geometry
# to changes in the 1D latent vector. It computes a finite-difference
# approximation of a local Lipschitz constant:
#
#     L_{i,j} = || geometry_i - geometry_j || / | latent_i - latent_j |
#
# it is evaluated over all xyz values on the grid 
# where:
#   - geometry_i is the SDF of the shape corresponding to latent_i,
#     represented as a flattened grid of values.
#   - geometry_j is the SDF for latent_j.
#   - || . || denotes the Euclidean (L2) norm over the flattened grid.
#
# Steps:
# 1. Build a fixed evaluation grid for SDF computation:
#       - 2D grid in x, y spanning [-1.8, 1.8] with resolution `grid_res` = 256
#       - z = 0 (planar slice through 3D space)
#       - Resulting grid size: grid_res x grid_res x 3 (xyz coordinates)
# 2. For each latent value:
#       - Compute the SDF on the grid using `eval_sdf(latent_vec)`
#       - Flatten the grid into a 1D vector of length grid_res^2
#       - Store these vectors in `sdf_cache`
# 3. Compute all pairwise differences between latent vectors:
#       - geom_diff = Euclidean norm of difference between SDF vectors
#       - latent_diff = absolute difference in latent values
#       - ratio = geom_diff / latent_diff
#         (approximates how much geometry changes per unit latent change)
# 4. Collect statistics:
#       - Mean, std, min, max of the ratios to quantify global Lipschitz behavior
# 5. Visualize with a histogram:
#       - Shows distribution of geometry sensitivity across the latent space
#
# Notes:
#   - The SDF grid is uniform; the L2 norm treats all grid points equally.
#   - Very small latent differences (<1e-8) are ignored to avoid division by zero.
#   - Higher ratios indicate more sensitive regions where small latent changes
#     cause large geometry changes.
#   - This analysis is currently for 1D latent vectors; for higher-dimensional
#     latent spaces, the latent norm should use a vector norm.

    print("\n[INFO] Computing latent Lipschitz geometry ratios...")
    sdf_cache = [eval_sdf(torch.tensor([z], dtype=torch.float32).to(device)).flatten() for z in latent_values]
    ratios = []
