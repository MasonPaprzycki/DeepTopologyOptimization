...
class DeepSDFTrainer:
    ...
    def __init__(
            ...
            lr_net=5e-4, 
            lr_latent=1e-3, 
            clamp_delta:float|None=0.1,
            regularize_latent: bool = True, 
    ...
    def train_step(
        ...
        if self.clamp_delta is not None:
            data_loss = clamped_l1_loss(pred, s_flat, self.clamp_delta).mean()
        else:
            data_loss = l1_loss(pred, s_flat).mean()
    ...
    def train(
        ...
        if stochastic_distribution:
            ...
            for epoch in range(1, epochs + 1):
                ...
                sigma = sigma0 * min(1.0, 1.0 / epoch)
        ...
        #Forced distibution mode (95% near surface, 5% far)
        ...
        for sid, pts, sdf in dataloader:
            ...
            for b in range(B):
                ...
                preprocessed.append((
                    sid_b,
                    pts_b[pos_near].unsqueeze(0),
                    pts_b[neg_near].unsqueeze(0),
                    pts_b[pos_far].unsqueeze(0),
                    pts_b[neg_far].unsqueeze(0),
                    sdf_b[pos_near].unsqueeze(0),
                    sdf_b[neg_near].unsqueeze(0),
                    sdf_b[pos_far].unsqueeze(0),
                    sdf_b[neg_far].unsqueeze(0),
                ))
    ...
    def train_via_grid_step_gradient_descent(
        ...
        not really necessary leaving in for conceptual interest and potential future use among a related method
        """
